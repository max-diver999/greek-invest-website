...
const LEAD_FROM_EMAIL = import.meta.env.LEAD_FROM_EMAIL || `Greek Invest <${SITE.email}>`;
...
async function sendAutoReply(to: string, name: string, context: string) {
  ...
  const topic = context ? escapeHtml(context) : 'your Greece property enquiry';
