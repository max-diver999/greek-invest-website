...
if (asJson) {
  ...
  if (!overdue.length && !malformed.length) {
    const next = claims.map((c) => c.reviewBy).sort()[0];
    console.log(next
      ? `\n✅ nothing overdue. Next review: ${next}`
      : '\n✅ nothing overdue — the register is empty. Foreign-jurisdiction figures on the comparison pages are unregistered, which means unwatched, not verified.');
  }
}
