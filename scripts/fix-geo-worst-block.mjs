#!/usr/bin/env node
/** Fix worst H2 block on each page still below GEO 90. */
...
for (const coll of ['guides', 'areas', 'compare']) {
  ...
  for (const name of readdirSync(dir).filter((f) => f.endsWith('.mdx'))) {
    ...
    const blocks = extractH2Blocks(body).map((b) => ({ b, s: scoreBlock(b, bodyPlain) }));
    blocks.sort((a, c) => a.s.overall - c.s.overall);
    const targets = blocks.filter((x) => x.s.overall < 90).slice(0, 3);
    ...
    let section = m[2];
    if (wordCount(stripMdx(worst.b.firstPara)) < 45 || !hasStat(worst.b.plainFirst)) {
    ...
    updated++;
    if (after >= TARGET || updated <= 5) console.log(`${name}: ${before} -> ${after}`);
  }
}
...
for (const coll of ['guides', 'areas', 'compare']) {
  for (const f of readdirSync(join(ROOT, 'src/content', coll)).filter((x) => x.endsWith('.mdx'))) {
    if (scorePage(parseMdxBody(readFileSync(join(ROOT, 'src/content', coll, f), 'utf8')), { collection: coll }).score >= 90) t90++;
  }
}
console.log(`Updated ${updated} files. Corpus 90+: ${t90}/114`);
