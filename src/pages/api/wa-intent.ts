...
function normalizeSiteUrl(value: unknown): string {
  ...
  try {
    ...
    if (host !== 'greek-invest.com' && !host.endsWith('.vercel.app')) {
      return '';
    }
    return url.href;
  } catch {
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const lines = [
      '💬 <b>WhatsApp click | greek-invest.com</b>',
    ...
    // WhatsApp click intents have no phone/email/name. Telegram only — no owner inbox email
    // (Kommo ingests LEAD_NOTIFY_EMAIL). Real leads go through /api/lead/ with valid phone.
