...
The yield gap is real and so is the reason for it. Cape Town's headline gross yields sit above the Greek equivalents, but they are earned in rand, on an emerging-market currency that can move enough between purchase and exit to erase the advantage entirely for a euro-based investor. Greek gross yields average 4.40% nationally and 5.43% in Athens, and Greek rental income tax then takes 15%, 25%, 35% and 45% by band on gross rent with only a flat 5% repairs deduction, so a 5.4% gross typically nets 1.7% to 3.0%. The honest comparison is therefore not yield against yield: it is a lower euro-denominated return that comes with an EU residence permit against a higher rand-denominated return that comes with none, since South African property confers no residency of any kind.
...
- **Greek net yield compression.** Greece's headline yields survive contact with tax far less well than the gross numbers suggest: 15% to 45% rental tax with no allowance, plus ENFIA, typically halves them.
